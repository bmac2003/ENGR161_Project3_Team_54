...
speed = -40             #General speed of the MACRO when moving forward
...
lapDirections = ['R','R','L','R','L','R','R',     #Lap 1 Turns
                 'R','R','L','L','R','L','R',     #Lap 2 Turns
                 'R','R','L','L','L','L']         #Lap 3 Turns
unloadAtBeacon = [False, False, True, False, False, False, True, 
...
def selectPath(junctionCount):
    ...
    if (lapDirections[junctionCount] == 'R'):  #Get directions from the directions array for given junction
        ...
        while (grovepi.digitalRead(line_finderL) == 1):  #Turn until the left sensor is completely acrsoss the line
            setTurnVector(upSpeed * -2, backSpeed * -0.5)
            trace()
            time.sleep(sleepTime)            
        '''
        ...
        while (grovepi.digitalRead(line_finderR) == 1):  #Turn until the right sensor is completely across the line
            setTurnVector(backSpeed * -0.5, upSpeed * -2)
            trace()
            time.sleep(sleepTime)
        '''
...
def unloadCargo():
    ...
    BP.set_motor_power(BP.PORT_A+BP.PORT_D, (speed + 10) * -1)
    time.sleep(1.25) #Move backwards to return to drop off spot
    BP.reset_all()

    return

def sportMode(): #ACTIVATE TURBOCHARGER
    ...
    while True:
        ...
        if not(mag['x']): #Check to unload if seeing a magnetic beacon
            ...
            if not(stillSeeingMag):
                beaconCount += 1
                #if (beaconCount == 3 or beaconCount == 7):
                    #speed -= 10
            stillSeeingMag = True
            turbo = True
            turboCount = 0
        ...
        if (gyro['x'] > gyroThreshold): #Boost over the hill when it is detected
            if (turbo and turboCount < 2):
                sportMode()
                turboCount += 1
                if (turboCount >= 2):
                    turbo = False
        elif (grovepi.ultrasonicRead(ultrasonic_ranger) < sightDistance): #Stop if seeing something within range
            print("Too Close")
        ...
        if (grovepi.digitalRead(line_finderL) == 1 and grovepi.digitalRead(line_finderR) == 1): #Select path
            junctionCount += selectPath(junctionCount)
        time.sleep(sleepTime)
